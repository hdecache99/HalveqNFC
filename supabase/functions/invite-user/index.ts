...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!caller) return new Response(JSON.stringify({ error: 'No autenticado' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    ...
    if (!membership || membership.role !== 'Administrador' || membership.status !== 'Activo') return new Response(JSON.stringify({ error: 'No tienes permisos para invitar usuarios' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
