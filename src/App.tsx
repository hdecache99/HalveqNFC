...
type LinktreeProfile = { id: string; name: string; client_name: string; slug: string; status: 'Activo' | 'Pausado'; store_id?: string }
type NfcTag = { id: string; name: string; client_name: string; destination_url: string; status: 'Activo' | 'Pausado'; scans: number; slug?: string; store_id?: string; profile_id?: string }
...
type ManagedLink = PublicLink & { tag_id?: string; profile_id: string; position: number; active: boolean }
...
function TagSetupView({ workspaceId, storeId, tags, profiles, setTags, showNotice }: { workspaceId: string | null; storeId: string | null; tags: NfcTag[]; profiles: LinktreeProfile[]; setTags: Dispatch<SetStateAction<NfcTag[]>>; showNotice: (message: string) => void }) {
  ...
  const createTag = async (event: FormEvent) => {
    ...
    if (profileMode === 'new') {
      const { data: profile, error: profileError } = await supabase.from('linktree_profiles').insert({ workspace_id: workspaceId, store_id: storeId, name, client_name: clientName, slug: safeSlug }).select('id').single()
      if (profileError || !profile) { showNotice(profileError?.message ?? 'No se pudo crear el perfil'); setSaving(false); return }
      profileId = profile.id
    }
    if (!profileId) { showNotice('Selecciona un perfil Linktree'); setSaving(false); return }
    const { data: tag, error } = await supabase.from('nfc_tags').insert({ workspace_id: workspaceId, store_id: storeId, profile_id: profileId, name, client_name: clientName, slug: safeSlug, destination_url: url || `${window.location.origin}/t/${safeSlug}` }).select('id, name, client_name, destination_url, status, slug, store_id, profile_id').single()
    if (error || !tag) { showNotice(error?.message ?? 'No se pudo crear el tag'); setSaving(false); return }
    if (url && profileMode === 'new') await supabase.from('nfc_links').insert({ tag_id: tag.id, profile_id: profileId, label: 'Visitar enlace', url, position: 0 })
  ...
  const profileSelector = <label>Perfil Linktree<select value={profileMode === 'new' ? 'new' : selectedProfileId} onChange={(event) => { const value = event.target.value; setProfileMode(value === 'new' ? 'new' : 'existing'); setSelectedProfileId(value === 'new' ? '' : value); const profile = profiles.find((item) => item.id === value); if (profile) { setName(profile.name); setClientName(profile.client_name) } }}><option value="new">Crear perfil nuevo</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.client_name} · {profile.name}</option>)}</select></label>
  return <div className="admin-page"><section className="page-heading"><div><p className="eyebrow">CONFIGURACIÓN NFC</p><h1>{tags.length ? 'Mis tags NFC' : 'Configura tu primer tag'} <span>✦</span></h1><p className="subheading">Cada tag abre una página de enlaces configurable y cuenta sus visitas.</p></div></section><div className="tag-setup-grid"><form className="panel setup-form" onSubmit={createTag}><h2>Crear tag NFC</h2><p className="modal-copy">Elige si este tag tendrá un Linktree nuevo o compartirá uno existente.</p>{profileSelector}<label>Nombre del tag<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Tag de entrada" required /></label><label>Negocio<input value={clientName} onChange={(event) => setClientName(event.target.value)} placeholder="La Esquina del Sabor" required /></label><label>Slug de la URL NFC<input value={slug} onChange={(event) => setSlug(event.target.value)} placeholder="entrada-principal" pattern="[a-zA-Z0-9-]+" required /></label>{profileMode === 'new' && <label>Primer enlace<input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://instagram.com/negocio" required /></label>}<button className="primary-button" type="submit" disabled={saving}>{saving ? 'Guardando...' : 'Crear tag NFC'} <Plus size={16} /></button></form><section className="panel setup-list"><div className="panel-header"><div><h2>Tags y perfiles</h2><p>Un perfil puede compartirse entre varios tags.</p></div></div>{tags.map((tag) => <div className="setup-tag" key={tag.id}><span className="tag-icon coral"><QrCode size={16} /></span><span><strong>{tag.name}</strong><small>{window.location.origin}/t/{(tag as NfcTag & { slug?: string }).slug ?? tag.id}</small></span><button className="outline-button" onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/t/${(tag as NfcTag & { slug?: string }).slug ?? tag.id}`).then(() => showNotice('URL copiada'))}>Copiar URL</button></div>)}{!tags.length && <p className="empty-state">Aún no hay tags configurados.</p>}</section></div></div>
}

function LinksView({ tags, profiles, links, setLinks, showNotice }: { tags: NfcTag[]; profiles: LinktreeProfile[]; links: ManagedLink[]; setLinks: Dispatch<SetStateAction<ManagedLink[]>>; showNotice: (message: string) => void }) {
  ...
  const saveLink = async (event: FormEvent) => {
    ...
    setSaving(true)
      const selectedProfile = tags.find((tag) => tag.id === selectedTag)?.profile_id
      if (!selectedProfile) { showNotice('Este tag todavía no tiene un perfil Linktree'); setSaving(false); return }
      const { data, error } = await supabase.from('nfc_links').insert({ tag_id: selectedTag, profile_id: selectedProfile, label: label.trim(), url: url.trim(), position: links.filter((item) => item.profile_id === selectedProfile).length }).select('id, tag_id, profile_id, label, url, position, active').single()
  ...
  return <div className="admin-page"><section className="page-heading"><div><p className="eyebrow">CONTENIDO POR TIENDA</p><h1>Enlaces <span>✦</span></h1><p className="subheading">Administra los enlaces del perfil que pueden compartir varios tags NFC.</p></div></section><div className="links-layout"><form className="panel link-form" onSubmit={saveLink}><h2>Agregar enlace</h2><p className="modal-copy">El enlace se publica en todos los tags asociados al mismo perfil.</p><label>Tag NFC<select value={selectedTag} onChange={(event) => setSelectedTag(event.target.value)} required>{tags.map((tag) => <option key={tag.id} value={tag.id}>{tag.client_name} · {tag.name}</option>)}</select></label><label>Texto del enlace<input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Instagram" required maxLength={80} /></label><label>URL<input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://instagram.com/negocio" required /></label><button className="primary-button" type="submit" disabled={saving || !tags.length}>{saving ? 'Guardando...' : 'Agregar enlace'} <Plus size={16} /></button></form><section className="panel managed-links"><div className="panel-header"><div><h2>Enlaces publicados</h2><p>{links.length} enlaces configurados en tus perfiles.</p></div></div>{profiles.map((profile) => <div className="link-group" key={profile.id}><div className="link-group-heading"><div><strong>{profile.client_name}</strong><small>{profile.name}</small></div><span>{links.filter((link) => link.profile_id === profile.id).length} enlaces</span></div>{links.filter((link) => link.profile_id === profile.id).map((link) => <div className="managed-link" key={link.id}><input value={link.label} onChange={(event) => setLinks((current) => current.map((item) => item.id === link.id ? { ...item, label: event.target.value } : item))} onBlur={() => void updateLink(link, { label: link.label })} /><input value={link.url} onChange={(event) => setLinks((current) => current.map((item) => item.id === link.id ? { ...item, url: event.target.value } : item))} onBlur={() => void updateLink(link, { url: link.url })} /><button className="row-action" onClick={() => void updateLink(link, { active: !link.active })} aria-label="Activar o pausar enlace"><i className={`link-state ${link.active ? 'on' : 'off'}`} /></button><button className="row-action" onClick={() => void deleteLink(link)} aria-label="Eliminar enlace"><X size={16} /></button></div>)}</div>)}{!tags.length && <p className="empty-state">Primero configura un tag NFC.</p>}</section></div></div>
}
...
function DashboardApp() {
  ...
  useEffect(() => {
    ...
    const loadWorkspace = async () => {
      ...
      setStoreId(store.id)
      const [membersResult, tagsResult, profilesResult] = await Promise.all([
        client.from('workspace_members').select('id, name, email, role, status').eq('workspace_id', membership.workspace_id).order('created_at'),
        (membership.role === 'Administrador' ? client.from('nfc_tags').select('id, name, client_name, destination_url, status, slug, store_id, profile_id').eq('workspace_id', membership.workspace_id) : client.from('nfc_tags').select('id, name, client_name, destination_url, status, slug, store_id, profile_id').eq('store_id', store.id)).order('created_at', { ascending: false }),
        (membership.role === 'Administrador' ? client.from('linktree_profiles').select('id, name, client_name, slug, status, store_id').eq('workspace_id', membership.workspace_id) : client.from('linktree_profiles').select('id, name, client_name, slug, status, store_id').eq('store_id', store.id)).order('created_at', { ascending: false }),
      ])
      if (membersResult.error || tagsResult.error || profilesResult.error) { setDataError(membersResult.error?.message ?? tagsResult.error?.message ?? profilesResult.error?.message ?? 'No se pudieron cargar los datos.'); setDataLoading(false); return }
      const tags = (tagsResult.data ?? []) as NfcTag[]
      setAdmins((membersResult.data ?? []) as AdminUser[])
      setFetchedTags(tags)
      setProfiles((profilesResult.data ?? []) as LinktreeProfile[])
      if (tags.length) {
        const profileIds = tags.map((tag) => tag.profile_id).filter((profileId): profileId is string => Boolean(profileId))
        const { data: linkRows, error: linksError } = await client.from('nfc_links').select('id, tag_id, profile_id, label, url, position, active').in('profile_id', profileIds).order('position')
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-wrap">
          {dataError && <div className="data-error">{dataError}</div>}{dataLoading && <div className="data-loading">Cargando datos reales...</div>}{activeNav === 'Administradores' ? <AdminsView admins={admins} setAdmins={setAdmins} showNotice={showNotice} workspaceId={workspaceId} /> : activeNav === 'Tags NFC' ? <TagSetupView workspaceId={workspaceId} storeId={storeId} tags={fetchedTags} profiles={profiles} setTags={setFetchedTags} showNotice={showNotice} /> : activeNav === 'Enlaces' ? <LinksView tags={fetchedTags} profiles={profiles} links={links} setLinks={setLinks} showNotice={showNotice} /> : <>
